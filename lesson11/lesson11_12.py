#老師版 練習使用者選取日期區間後可以顯示其選擇的日期區間 (0817表格不美麗未完成版)
#用darasource1.py
import datetime
...
def get_stations():
    """取得車站資料"""
    return datasource1.get_stations_names()

@st.cache_data
def get_date_range():
    """取得日期範圍"""
    return datasource1.get_min_and_max_date()
...
# 轉換為 datetime.date（如果 datasource1 回傳字串）
try:
...
# 請使用datasource1.get_station_data_by_date 函數取得資料,並顯示資料
st.write("您選擇的車站:", station)
st.write("日期範圍:", start_date, "至", end_date)

data = datasource1.get_station_data_by_date(station, start_date, end_date)
if data is None:
    st.error("無法取得車站資料，請稍後再試。")
else:
    st.write("進出站人數資料:")
